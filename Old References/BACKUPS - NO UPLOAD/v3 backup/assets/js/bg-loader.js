(function() {
  ...
  function triggerScanlineSweep() {
    ...
    // Rarely trigger the old sweep effect
    if (Math.random() < 0.08) { // 8% chance on transition
      scanSweep.classList.remove('run');
      void scanSweep.offsetWidth;
      scanSweep.classList.add('run');
    }
    
    // More commonly trigger CRT retrace (more realistic)
    const crtRetrace = document.getElementById('crtRetrace');
    if (crtRetrace && Math.random() < 0.25) { // 25% chance
      crtRetrace.classList.remove('run');
      void crtRetrace.offsetWidth;
      crtRetrace.classList.add('run');
    }
    
    // Very rare tracking errors on transitions
    if (Math.random() < 0.02) { // 2% chance
      const elements = document.querySelectorAll('header, nav');
      elements.forEach(el => {
        if (Math.random() < 0.15) { // 15% chance
          el.classList.add('horizontal-hold-error');
          setTimeout(() => el.classList.remove('horizontal-hold-error'), 1800);
        }
      });
    }
  }
