...
(function() {
  ...
  // Enhanced positioning with stable dimensions
  function positionMini(mini) {
    ...
    /* Check if this is initial positioning */
    const isInitialPositioning = mini.dataset.initialized !== 'true';
    
    /* Only set dimensions during initial positioning */
    if (isInitialPositioning) {
      ...
      /* Store initial dimensions for consistency */
      const rect = mini.getBoundingClientRect();
      mini.dataset.initialWidth = rect.width;
      mini.dataset.initialHeight = rect.height;
      mini.dataset.initialized = 'true';
      
      /* Apply fixed width to prevent content-based resizing */
      mini.style.width = `${rect.width}px`;
    }
    
    /* Use stored dimensions if available */
    const width = parseFloat(mini.dataset.initialWidth) || mini.offsetWidth;
    const height = parseFloat(mini.dataset.initialHeight) || mini.offsetHeight;
    
    /* Center with constraints and footer clearance */
    let left = Math.max(padding, (viewportWidth - width) / 2);
    let top = Math.max(padding, (viewportHeight - height - footerHeight) / 2);
    
    /* Ensure doesn't exceed bounds */
    left = Math.min(left, viewportWidth - width - padding);
    top = Math.min(top, viewportHeight - height - footerHeight - padding);
    ...
    console.log(`[Mini] Positioned at ${Math.round(left)}x${Math.round(top)}, size ${Math.round(width)}x${Math.round(height)}`);
  }
