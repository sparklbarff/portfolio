...
(function() {
  ...
  // NEW: Phosphor persistence simulation
  function addPhosphorPersistence(letter, intensity) {
    // Add burn-in effect for high intensity glitches
    if (intensity > 0.6) {
      letter.element.classList.add('phosphor-burn');
      
      // Persistent glow that fades slowly
      setTimeout(() => {
        letter.element.classList.remove('phosphor-burn');
  ...
  // NEW: Scanline interaction - bright content affects nearby scanlines
  function triggerScanlineInteraction(letter, intensity) {
    if (intensity < 0.4) return;
    
    const scanlineInteraction = document.getElementById('scanlineInteraction');
    if (!scanlineInteraction) return;
    ...
    // Create localized scanline brightening
    scanlineInteraction.style.opacity = intensity * 0.5;
    scanlineInteraction.style.background = `
      radial-gradient(
        circle at ${centerX}% ${centerY}%,
        rgba(0, 255, 200, ${intensity * 0.3}) 0%,
        rgba(232, 227, 216, ${intensity * 0.2}) 15%,
        rgba(0, 255, 200, ${intensity * 0.1}) 30%,
        transparent 50%
      )
    `;
  ...
  function applyOptimizedGlitch(letter, intensity) {
    ...
    // Enhanced RGB separation with phosphor colors
    if (intensity > 0.4) {
      ...
      letter.element.style.textShadow = `
        ...
        0 0 ${intensity * 8}px rgba(232, 227, 216, ${intensity * 0.4}),
        0 0 ${intensity * 16}px rgba(0, 255, 200, ${intensity * 0.2})
      `;
    }
    ...
    if (intensity > 0.3) {
      const brightness = 0.85 + (intensity * 0.4); /* INCREASED brightness variation */
      letter.element.style.filter = `brightness(${brightness}) contrast(${1 + intensity * 0.2})`;
    }
