(function() {
  ...
  function calculateMagneticDistortion(linkState, globalIntensity) {
    const localPhase = globalMagneticPhase + (linkState.normalizedX * Math.PI * 1.7);
  ...
  function applyTrackingError() {
    ...
    linkStates.forEach(linkState => {
      ...
      linkState.element.style.filter = 'brightness(1.4) contrast(1.8) saturate(0.7)';
    });
  ...
  function addPhosphorPersistence(linkState, char, intensity) {
    ...
    linkState.glitchState.persistenceLayers = linkState.glitchState.persistenceLayers
      .filter(layer => (now - layer.startTime) < crtSettings.phosphorDecay * 2);
  }
  ...
  function applyRealisticDistortion(linkState, intensity) {
    ...
    if (intensity > 0.4) {
      ...
      const blueOffset = convergenceError * (1.2 + (1 - linkState.normalizedX) * 0.8);
      ...
      if (intensity > 0.6) {
        addPhosphorPersistence(linkState, linkState.element.textContent, intensity);
      }
    }
  ...
  function runEnhancedGlitchSystem(currentTime) {
    ...
    const baseIntensity = Math.random() < 0.15 ? 0.7 + Math.random() * 0.3 : Math.random() * 0.4;
    ...
    linkStates.forEach(linkState => {
      ...
      if (localIntensity > 0.15) {
        ...
        if (linkState.glitchState.active) {
          const timeSinceGlitch = currentTime - linkState.glitchState.lastGlitchTime;
