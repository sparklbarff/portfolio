...
(function() {
  ...
  function addPersistenceLayer(letter, intensity) {
    ...
    letter.glitchState.persistenceLayers = letter.glitchState.persistenceLayers
      .filter(layer => (now - layer.startTime) < settings.phosphorDecayTime);
  }
  ...
  function updatePhosphorDecay(letter) {
    ...
    layers.forEach(layer => {
      ...
      if (layerIntensity > 0.05) {
        ...
        shadowLayers.push({
          ...
          color: layerIntensity > 0.5 ? phosphorColors.green : phosphorColors.blue
        });
      }
    });
    ...
    if (totalIntensity > 0.02) {
      ...
      const shadows = shadowLayers.map(layer => 
        `${layer.offset}px 0 ${layer.color.replace('0.9)', `${layer.opacity})`)}`
      );
  ...
  function updateSyncErrors() {
    ...
    if (Math.random() < settings.syncInstability) {
      ...
      setTimeout(() => { horizontalSyncError = 0; }, 50 + Math.random() * 100);
    }
  ...
  function applyEnhancedGlitch(letter, intensity) {
    ...
    letter.element.style.transform = 
      `translate(${totalX}px, ${totalY}px) rotate(${rotation}deg)`;
  ...
  function runRealisticGlitchLoop(currentTime) {
    ...
    const baseIntensity = isStrongGlitch ? 0.8 + Math.random() * 0.2 : Math.random() * 0.3;
