...
(function() {
  ...
  function applyOptimizedDistortion(linkState, baseIntensity) {
    ...
    /* Magnetic distortion applied only to individual link element */
    ...
    /* Ensure transform isolation - prevents affecting nav container */
  ...
  // ADD MISSING TEMPORAL BEHAVIOR FUNCTIONS (Complete implementations)
  ...
  function maybeStartNavGlitchStorm() {
    ...
    if (!navGlitchStormActive && state.mode === "failure" && Math.random() < 0.15 + state.thermalLevel * 0.5) {
      ...
      console.log('[Nav] Glitch storm started');
      
      const stormTimer = setTimeout(() => { 
        navGlitchStormActive = false; 
        console.log('[Nav] Glitch storm ended');
      }, 1500 + Math.random() * 2000);
      NavCleanup.registerTimer(stormTimer);
    }
  }
  ...
  function applyViolentNavGlitch(linkState, intensity) {
    if (Math.random() < 0.5) {
      ...
      const shakeTimer = setTimeout(() => { linkState.element.style.animation = ''; }, 180);
      NavCleanup.registerTimer(shakeTimer);
    }
  ...
  function maybeSurpriseNavGlitch(linkState, intensity) {
    ...
    if (state.mode === "stable" && Math.random() < 0.04 + state.thermalLevel * 0.08) {
      ...
      console.log('[Nav] Surprise glitch triggered');
    }
  }

  function isPersistent(problem) {
    const state = window.CRTTemporalState;
    return state && state.persistentProblems.has(problem) && state.mode === "failure";
  }
