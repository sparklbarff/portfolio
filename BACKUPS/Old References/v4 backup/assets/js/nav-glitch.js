...
(function() {
  ...
  function updatePerformanceSettings() {
    ...
    switch (performanceLevel) {
      case 'low':
        crtSettings.glitchChance = 0.08;
        crtSettings.maxSimultaneousGlitches = 1;
        break;
      case 'medium':
  ...
  // Phase 4: Navigation-specific authenticity enhancements
  const NAV_AUTHENTICITY = {
    MAGNETIC_SATURATION_THRESHOLD: 0.8,  // Navigation coils saturate earlier
    THERMAL_RESPONSE_FACTOR: 1.3,        // Navigation is more thermally sensitive
    CONVERGENCE_PRIORITY_MULTIPLIER: 1.5, // Navigation requires better convergence
    FREQUENCY_RESPONSE_CURVE: 0.7        // Navigation frequency handling
  };

  // Enhanced magnetic distortion for navigation with priority convergence
  function calculateAuthenticNavDistortion(linkState, globalIntensity) {
    const crtState = window.CRTTemporalState;
    const thermalAdjustment = 1 + (crtState.thermalLevel * NAV_AUTHENTICITY.THERMAL_RESPONSE_FACTOR);
    
    const localPhase = globalMagneticPhase + (linkState.normalizedX * Math.PI);
    const wearMultiplier = 1 + navWearPatterns.groupMagneticWear * 140; // More sensitive to wear
    const fieldIntensity = globalIntensity * NAV_CONSTANTS.MAGNETIC_FIELD_STRENGTH * wearMultiplier * thermalAdjustment;
    
    // Navigation priority magnetic field calculation
    const primaryField = Math.sin(localPhase) * fieldIntensity;
    const secondaryField = Math.cos(localPhase * 1.4 + navWearPatterns.thermalStress * 18) * fieldIntensity * 0.65;
    const thermalDrift = Math.sin(localPhase * 0.35) * fieldIntensity * navWearPatterns.thermalStress * 12;
    
    // Enhanced chaos field for navigation prominence
    const chaosField = Math.sin(localPhase * 3.2 + Math.random() * Math.PI) * fieldIntensity * 0.6;
    
    // Navigation-specific convergence correction (better than title)
    const convergenceCorrection = Math.pow(linkState.normalizedX - 0.5, 2) * 0.15;
    
    // Calculate displacement with navigation priority
    const xDisplace = (primaryField + secondaryField + thermalDrift + chaosField - convergenceCorrection) * 45;
    const yDisplace = (secondaryField * 0.75 + thermalDrift + chaosField * 0.65) * 28;
    const rotation = (primaryField * 0.25 + thermalDrift + chaosField * 0.45) * 20;
    
    // Apply saturation limiting for navigation stability
    const saturationLimit = NAV_AUTHENTICITY.MAGNETIC_SATURATION_THRESHOLD * 50;
    return {
      xDisplace: Math.max(-saturationLimit, Math.min(saturationLimit, xDisplace)),
      yDisplace: Math.max(-saturationLimit, Math.min(saturationLimit, yDisplace)),
      rotation: Math.max(-15, Math.min(15, rotation))
    };
  }

  // Enhanced navigation color effects with frequency response
  function applyAuthenticNavColorEffects(linkState, effectIntensity) {
    const crtState = window.CRTTemporalState;
    
    if (effectIntensity < NAV_THRESHOLDS.RGB_SEPARATION) return;
    
    // Navigation frequency response curve
    const frequencyResponse = Math.pow(effectIntensity, NAV_AUTHENTICITY.FREQUENCY_RESPONSE_CURVE);
    
    // Thermal color temperature shift for navigation
    const thermalShift = crtState.thermalLevel * 0.4;
    const hueShift = (Math.random() - 0.5) * frequencyResponse * 80 + thermalShift * 20;
    const saturation = 0.8 + frequencyResponse * 0.8;
    const brightness = 0.7 + frequencyResponse * 0.6;
    
    linkState.element.style.filter = `
      hue-rotate(${hueShift}deg) 
      saturate(${saturation})
      brightness(${brightness})
      contrast(${1 + frequencyResponse * 0.4})
      sepia(${thermalShift * 0.25})
    `;
  ...
  // Enhanced navigation phosphor effects with navigation priority
  function addAuthenticNavPhosphorEffects(linkState, effectIntensity) {
    const crtState = window.CRTTemporalState;
    
    if (effectIntensity > 0.5) { // Lower threshold for navigation
      linkState.element.classList.add('phosphor-glow');
      
      // Navigation-specific phosphor intensity
      const navPhosphorIntensity = effectIntensity * NAV_AUTHENTICITY.CONVERGENCE_PRIORITY_MULTIPLIER;
      
      // Enhanced navigation glow with priority wavelengths
      linkState.element.style.textShadow = `
        0 0 2px currentColor,
        0 0 6px rgba(255, 64, 32, ${navPhosphorIntensity * 0.7}),
        0 0 12px rgba(32, 255, 64, ${navPhosphorIntensity * 0.8}),
        0 0 24px rgba(64, 128, 255, ${navPhosphorIntensity * 0.6}),
        0 0 36px rgba(232, 227, 216, ${navPhosphorIntensity * 0.5}),
        0 0 72px rgba(0, 255, 200, ${navPhosphorIntensity * 0.2})
      `;
      ...
      // Track enhanced phosphor wear for navigation
      const linkIndex = linkStates.indexOf(linkState);
      const currentWear = navWearPatterns.linkWear.get(linkIndex) || 0;
      navWearPatterns.linkWear.set(linkIndex, currentWear + navPhosphorIntensity * 0.0008);
    }
  }

  // Enhanced navigation color bleeding with navigation-specific range
  function applyAuthenticNavColorBleeding(linkState, effectIntensity) {
    const crtState = window.CRTTemporalState;
    ...
    // Thermal expansion of bleed range for navigation
    const thermalExpansion = 1 + crtState.thermalLevel * 1.5;
    const bleedRange = Math.min(4, Math.floor(NAV_CONSTANTS.COLOR_BLEED_RANGE * thermalExpansion));
    
    linkStates.forEach((sibling, index) => {
      const distance = Math.abs(index - currentIndex);
      if (distance <= bleedRange && distance > 0) {
        const bleedIntensity = effectIntensity * 0.35 * (1 / Math.pow(distance, 0.75));
        
        if (bleedIntensity > 0.08) {
          // Navigation-specific frequency response
          const frequencyFactor = 1 - (distance * NAV_AUTHENTICITY.FREQUENCY_RESPONSE_CURVE * 0.2);
          const effectiveBleed = bleedIntensity * Math.max(0.4, frequencyFactor);
          
          const hueShift = (Math.random() - 0.5) * 50 * effectiveBleed;
          sibling.element.style.filter = `
            hue-rotate(${hueShift}deg) 
            saturate(${1.2 + effectiveBleed * 0.9})
            brightness(${1 + effectiveBleed * 0.5})
          `;
  ...
  // Main navigation distortion with authentic physics
  function applyAuthenticNavDistortion(linkState, baseIntensity) {
    const crtState = window.CRTTemporalState || { mode: "stable", thermalLevel: 0 };
    
    // Navigation-specific intensity scaling
    let effectiveIntensity = baseIntensity;
    if (crtState.mode === "stable") {
      effectiveIntensity *= 0.7 - crtState.thermalLevel * 0.15; // Navigation more stable
    } else {
      effectiveIntensity *= 1.1 + crtState.thermalLevel * 0.4; // But more sensitive to failure
    }

    // Apply navigation temporal behaviors
    maybeStartNavGlitchStorm();
    if (navGlitchStormActive || (crtState.mode === "failure" && Math.random() < 0.2 + crtState.thermalLevel * 0.25)) {
      applyViolentNavGlitch(linkState, effectiveIntensity * 1.3);
    }
    maybeSurpriseNavGlitch(linkState, effectiveIntensity);

    // Navigation persistent problems
    if (isPersistent("scanlineWarp")) {
      linkState.element.style.transform += " skewY(" + ((Math.random() - 0.5) * 2) + "deg)";
    }
    if (isPersistent("colorBleed")) {
      applyAuthenticNavColorBleeding(linkState, effectiveIntensity * 1.4);
    }
    if (isPersistent("phosphorBurn")) {
      linkState.element.classList.add('phosphor-burn');
      const burnTimer = setTimeout(() => linkState.element.classList.remove('phosphor-burn'), 1000 + Math.random() * 1200);
      NavCleanup.registerTimer(burnTimer);
    }

    // Core authentic navigation distortion
    const distortion = calculateAuthenticNavDistortion(linkState, effectiveIntensity);
    linkState.element.style.transform =
      `translate(${distortion.xDisplace}px, ${distortion.yDisplace}px) rotate(${distortion.rotation}deg)`;
    
    // Enhanced RGB separation for navigation priority
    if (effectiveIntensity > NAV_THRESHOLDS.RGB_SEPARATION) {
      const convergenceError = NAV_CONSTANTS.CONVERGENCE_ERROR_BASE * NAV_AUTHENTICITY.CONVERGENCE_PRIORITY_MULTIPLIER;
      const positionFactor = 1.3 + linkState.normalizedX * 0.8;
      const redOffset = convergenceError * positionFactor;
      const blueOffset = convergenceError * (1.3 + (1 - linkState.normalizedX) * 0.8);
      
      const shadowLayers = [
        `${redOffset}px 0 ${phosphorColors.red}`,
        `${-blueOffset}px 0 ${phosphorColors.blue}`,
        `0 0 ${effectiveIntensity * 22}px rgba(232, 227, 216, ${effectiveIntensity * 0.7})`,
        `0 0 ${effectiveIntensity * 44}px rgba(0, 255, 200, ${effectiveIntensity * 0.4})`
      ];
      
      linkState.element.style.textShadow = shadowLayers.join(', ');
    }
    
    // Navigation character replacement with enhanced wear
    const linkWear = navWearPatterns.linkWear.get(linkStates.indexOf(linkState)) || 0;
    const replaceChance = 0.75 + linkWear * 5;
    ...
    if (effectiveIntensity > NAV_THRESHOLDS.CHARACTER_REPLACE && Math.random() < replaceChance) {
      ...
      for (let i = 0; i < text.length; i++) {
        if (Math.random() < (effectiveIntensity * 0.8 + linkWear)) {
          newText += glitchChars[Math.floor(Math.random() * glitchChars.length)];
        } else {
          newText += text[i];
        }
      ...
      }, NAV_TIMINGS.RESTORE_DELAY_BASE + Math.random() * 80);
      NavCleanup.registerTimer(restoreTimer);
    }
    
    // Navigation brightness modulation with priority scaling
    if (effectiveIntensity > NAV_THRESHOLDS.BRIGHTNESS_MOD) {
      const brightness = 0.65 + (effectiveIntensity * 0.9);
      const contrast = 1 + (effectiveIntensity * 0.7);
      linkState.element.style.filter = `brightness(${brightness}) contrast(${contrast})`;
    }
    
    // Apply all authentic navigation effects
    applyAuthenticNavColorEffects(linkState, effectiveIntensity);
    applyAuthenticNavColorBleeding(linkState, effectiveIntensity);
    applyNavInterlacing(linkState, effectiveIntensity);
    addAuthenticNavPhosphorEffects(linkState, effectiveIntensity);
    
    // Navigation cascade trigger
    if (effectiveIntensity > NAV_THRESHOLDS.CASCADE_TRIGGER) {
      triggerGroupCascadeFailure(effectiveIntensity);
    }
  }

  // Update main navigation loop to use authentic distortion
  function runOptimizedGlitchSystem(currentTime) {
    updatePerformanceSettings();
    ...
    const state = window.CRTTemporalState || { mode: "stable", thermalLevel: 0 };
    const systemWear = Math.min(navWearPatterns.groupMagneticWear * 10, 0.7);
    const isStrongGlitch = Math.random() < 0.35;
    
    // Navigation priority base intensity
    const baseIntensity = isStrongGlitch ? 
      (0.85 + Math.random() * 0.6) : // Navigation can handle higher intensity
      (Math.random() * 0.6 + systemWear);
    ...
    linkStates.forEach(linkState => {
      ...
      if (shouldGlitch) {
        const localIntensity = baseIntensity * (0.6 + Math.random() * 0.8);
        applyAuthenticNavDistortion(linkState, localIntensity);
        linkState.glitchState.active = true;
        linkState.glitchState.intensity = localIntensity;
        linkState.glitchState.lastGlitch = currentTime;
        activeGlitches++;
