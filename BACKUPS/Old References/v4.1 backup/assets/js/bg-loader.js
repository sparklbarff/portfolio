...
(function() {
  ...
  function analyzeBackgroundIntensity(imageSrc) {
    ...
    img.onload = function() {
      ...
      try {
        ...
        console.warn('[Background] Could not analyze brightness:', e);
      }
    };
  ...
  function fetchManifest() {
    console.log('[Background] Fetching manifest...');
    return fetch('manifest.json')
      ...
      .then(data => {
        console.log('[Background] Manifest loaded:', data);
        return data;
      })
      .catch(error => {
  ...
  function generateImagePaths() {
    if (!manifest || manifest.count <= 0) {
      console.error('[Background] Invalid manifest');
      return [];
    }
  ...
  function preloadNext(count) {
    ...
    // Adjust preload count based on performance
    const adjustedCount = BG_CONSTANTS.PRELOAD_COUNTS[performanceLevel.toUpperCase()] || count;
  ...
  function setupFirstBg() {
    ...
    return preloadImage(firstSrc)
      ...
      .catch(error => {
        ...
        // Create standardized fallback background
        ...
        console.log('[Background] Created fallback background');
      });
  }
  ...
  function initBackgrounds() {
    ...
    return fetchManifest()
      ...
      .catch(error => {
        ...
        // Emergency fallback with standardized styling
        const fallback = document.createElement('div');
        fallback.className = 'bg-image active';
        fallback.style.background = '#2b2b2b';
        fallback.style.filter = 'none';
