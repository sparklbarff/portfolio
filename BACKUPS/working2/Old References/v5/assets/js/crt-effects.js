...
(function() {
  ...
  // Preload and optimize critical effects
  function preloadCriticalEffects() {
    const criticalEffects = [
      { id: 'crtRetrace', name: 'CRT Retrace' },
      { id: 'vhsSweep', name: 'VHS Sweep' },
      { id: 'vhsTracking', name: 'VHS Tracking' },
      { id: 'vhsHeadSwitch', name: 'VHS Head Switch' }
    ];
    
    criticalEffects.forEach(({ id, name }) => {
      ...
      if (element) {
        ...
        // Trigger initial state to compile shaders
        element.classList.add('active');
        setTimeout(() => element.classList.remove('active'), 50);
        
        console.log(`[Effects] Preloaded ${name}`);
      }
    });
    
    // Precompile backdrop filters by creating temporary element
    const filterTest = document.createElement('div');
    filterTest.style.cssText = `
      position: absolute;
      top: -1000px;
      left: -1000px;
      width: 1px;
      height: 1px;
      backdrop-filter: brightness(1.2) blur(0.3px) contrast(1.1);
      opacity: 0;
    `;
    document.body.appendChild(filterTest);
    
    setTimeout(() => {
      if (filterTest.parentNode) {
        filterTest.parentNode.removeChild(filterTest);
      }
    }, 100);
  }
  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    const initTimer = setTimeout(() => {
      try {
        // Preload effects first
        preloadCriticalEffects();
        
        // Then start effect systems
        briefPhosphorGlow();
        vhsTrackingErrors();
