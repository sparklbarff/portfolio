...
(function() {
  ...
  function updatePerformanceSettings() {
    ...
    // Adjust settings based on performance level - MORE AGGRESSIVE
    switch (performanceLevel) {
      case 'low':
        settings.glitchChance = 0.08; // INCREASED from 0.05
        settings.strongGlitchChance = 0.04; // INCREASED from 0.02
        break;
      case 'medium':
        settings.glitchChance = 0.15; // INCREASED from 0.08
        settings.strongGlitchChance = 0.08; // INCREASED from 0.04
        break;
      default:
        settings.glitchChance = 0.25; // INCREASED from 0.15
        settings.strongGlitchChance = 0.15; // INCREASED from 0.08
    }
  }
  ...
  // CRT simulation configuration - INCREASED CHAOS
  const settings = {
    magneticFieldStrength: CRT_CONSTANTS.MAGNETIC_FIELD_STRENGTH,
    convergenceError: CRT_CONSTANTS.CONVERGENCE_ERROR_BASE,
    glitchChance: 0.22, // INCREASED from 0.15 for more chaos
    strongGlitchChance: 0.12, // INCREASED from 0.08 for more chaos
    colorTrackingIntensity: 0.25,           // Chroma tracking error strength
    chromaSkewChance: 0.25, // INCREASED from 0.18 for more color chaos
    colorVibrateChance: 0.35, // INCREASED from 0.25 for more vibration
    colorFadeChance: 0.18, // INCREASED from 0.12 for more fading
    colorBleedIntensity: 0.3, // INCREASED from 0.2 for more bleeding
    phosphorTrailLength: CRT_CONSTANTS.PHOSPHOR_TRAIL_LENGTH,
    interlaceFlickerChance: 0.4 // INCREASED from 0.3 for more flicker
  };
  ...
  function runOptimizedGlitchLoop(currentTime) {
    ...
    // MAXIMUM organic intensity with more chaos
    const wearInfluence = Math.min(wearPatterns.magneticWear * 20, 0.7); // INCREASED multiplier
    const isStrongGlitch = Math.random() < (settings.strongGlitchChance + wearInfluence);
    
    // MAXIMUM base intensity for extreme chaos
    const baseIntensity = isStrongGlitch ? 
      (0.95 + Math.random() * 0.6) : // INCREASED maximum chaos
      (Math.random() * 0.6 + wearInfluence); // INCREASED minimum
    ...
    letters.forEach(letter => {
      ...
      // MAXIMUM organic glitch probability
      const organicGlitchChance = settings.glitchChance + 
        wearLevel * 0.3 + // INCREASED from 0.2
        wearPatterns.thermalStress * 0.15 + // INCREASED from 0.1
        wearPatterns.powerSupplyWear * 0.2; // INCREASED from 0.15
      
      const shouldGlitch = Math.random() < organicGlitchChance && 
                          activeGlitches < maxActiveGlitches &&
                          timeSinceLastGlitch > (35 - wearLevel * 50); // FASTER glitching - reduced from 50
  ...
  // Phase 4: Visual Authenticity Enhancement - SINGLE DECLARATION
  ...
  // Intersection Observer for efficient trail cleanup
  let trailObserver = null;
  
  function initTrailObserver() {
    if (!('IntersectionObserver' in window)) return;
    
    trailObserver = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (!entry.isIntersecting) {
          // Trail has moved out of viewport, clean it up
          const trailId = entry.target.id;
          if (phosphorTrails.has(trailId)) {
            entry.target.remove();
            phosphorTrails.delete(trailId);
          }
        }
      ...
      rootMargin: '100px', // Clean up when 100px outside viewport
      threshold: 0
    });
  }

  // Preload critical effects on first interaction
  function preloadCriticalEffects() {
    const criticalEffects = ['crtRetrace', 'vhsSweep', 'vhsTracking'];
    criticalEffects.forEach(id => {
    ...
    // Preload glow effects by creating hidden elements
    const glowPreload = document.createElement('div');
    glowPreload.style.cssText = 'position:absolute;top:-1000px;left:-1000px;opacity:0;pointer-events:none;';
    glowPreload.className = 'tg-letter phosphor-glow';
    glowPreload.textContent = 'X';
    document.body.appendChild(glowPreload);
    
    // Remove after brief display to trigger shader compilation
    setTimeout(() => {
      if (glowPreload.parentNode) {
        glowPreload.parentNode.removeChild(glowPreload);
      }
    }, 100);
    
    console.log('[Title] Critical effects preloaded');
  }
  ...
  function createAuthenticPhosphorTrail(letter, effectIntensity) {
    ...
    // Add to intersection observer for efficient cleanup
    if (trailObserver) {
      trailObserver.observe(trail.element);
    }
    ...
    function animatePhosphorDecay() {
      ...
      if (!currentStage) {
        // Decay complete
        if (trailObserver) trailObserver.unobserve(trail.element);
        if (trail.element.parentNode) {
          trail.element.parentNode.removeChild(trail.element);
        }
        phosphorTrails.delete(trailId);
