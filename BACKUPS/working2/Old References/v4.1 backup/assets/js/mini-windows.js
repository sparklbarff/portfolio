...
(function() {
  ...
  function positionMini(mini) {
    ...
    const padding = Math.min(viewportWidth * 0.05, 40); // 5% padding, max 40px
    const maxWidth = viewportWidth - (padding * 2);
    const maxHeight = viewportHeight - (padding * 2);
    
    // Ensure mini doesn't exceed max dimensions
    if (rect.width > maxWidth) {
      mini.style.width = `${maxWidth}px`;
    }
    if (rect.height > maxHeight) {
      mini.style.maxHeight = `${maxHeight}px`;
    }
    
    // Recalculate after potential resizing
    const finalRect = mini.getBoundingClientRect();
    
    // Center horizontally and vertically with constraints
    let left = (viewportWidth - finalRect.width) / 2;
    let top = (viewportHeight - finalRect.height) / 2;
    
    // Ensure minimum padding from edges
    left = Math.max(padding, Math.min(left, viewportWidth - finalRect.width - padding));
    top = Math.max(padding, Math.min(top, viewportHeight - finalRect.height - padding));
    
    // Apply safe area adjustments for mobile devices
    const safeAreaTop = parseInt(getComputedStyle(document.documentElement).getPropertyValue('--safe-area-inset-top') || '0');
    const safeAreaBottom = parseInt(getComputedStyle(document.documentElement).getPropertyValue('--safe-area-inset-bottom') || '0');
    
    if (safeAreaTop > 0) top = Math.max(top, safeAreaTop + padding);
    if (safeAreaBottom > 0) top = Math.min(top, viewportHeight - finalRect.height - safeAreaBottom - padding);
    
    mini.style.left = `${left}px`;
    mini.style.top = `${top}px`;
    
    console.log(`[Mini] Positioned at ${left}x${top}, size ${finalRect.width}x${finalRect.height}`);
  }
