  ...
  const PerformanceMonitor = {
    ...
    detectDevice() {
      try {
        ...
        const isMediumConnection = connection && (connection.effectiveType === '3g');
        ...
        const isLowGPU = renderer.includes('Mali') || renderer.includes('Adreno 3') || renderer.includes('PowerVR');
        
        // Determine overall performance level
        if (isLowMemory || isSlowConnection || isLowCore || isLowGPU || (isMobile && isTouch)) {
    ...
    startRealTimeMonitoring() {
      ...
      const monitorFrame = (currentTime) => {
        ...
        if (frameCount % 60 === 0) {
          ...
          const targetFPS = PERF_FRAME_RATES[this.performanceLevel.toUpperCase()];
    ...
    applyAdaptiveOptimizations() {
      ...
      docEl.classList.remove('perf-low', 'perf-medium', 'perf-high', 'perf-reduce');
      ...
      window.dispatchEvent(new CustomEvent('performanceAdapted', {
    ...
    setupEventListeners() {
      ...
      if (navigator.connection) {
        navigator.connection.addEventListener('change', () => {
          console.log('[Performance] Network change detected, re-evaluating...');
          this.detectDevice();
          this.applyAdaptiveOptimizations();
        });
      }
    ...
    applyInitialOptimizations() {
      ...
      console.log(`[Performance] Applied ${this.performanceLevel} optimizations`);
    },
    ...
    adjustAnimationDurations(level) {
      ...
      document.documentElement.style.setProperty('--fade-dur', `${9 * multiplier}s`);
      document.documentElement.style.setProperty('--period', `${20 * multiplier}s`);
      document.documentElement.style.setProperty('--sweep-dur', `${25 * multiplier}s`);
    },
