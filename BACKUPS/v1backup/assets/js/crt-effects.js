...
(function () {
  ...
  function coordinatePhysicsEffects() {
    if (!window.CRTPhysics || !shouldRun()) return;
    ...
    try {
      ...
      // Apply temperature-based color shifting via CSS custom properties
      ...
      // Apply phosphor-based effects when particles are active
      ...
      // Apply convergence-based screen distortion
      const convergenceIntensity = Math.abs(
        parseFloat(
          convergenceError.red.textShadow?.match(/([-\d.]+)px/)?.[1] || 0
        )
      );
    ...
    if (directorAvailable && window.CRTDirector) {
      ...
      if (crtState) {
        ...
        if (crtState.cascadeLevel > 0)
          chance *= 1 + crtState.cascadeLevel * 0.7; // Reduced from 1.0
