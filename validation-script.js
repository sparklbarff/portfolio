...
console.log(`  🧠 Memory (if available): ${navigator.deviceMemory || 'Unknown'} GB`);
console.log(`  ⚡ Connection (if available): ${navigator.connection?.effectiveType || 'Unknown'}`);
...
if (window.bgLoader) {
    ...
    console.log(`  📊 Browser Support:`, bgLoader.browserSupport || 'Not available');
    console.log(`  📋 Manifest Status:`, bgLoader.manifest ? 'Loaded' : 'Loading/Failed');
...
const requiredElements = {
    ...
    'main-content': document.querySelector('main') || document.querySelector('.main')
};
...
const customPropValue = getComputedStyle(testElement).getPropertyValue('--test-prop');
document.body.removeChild(testElement);

console.log(`  ${customPropValue.trim() === 'test-value' ? '✅' : '❌'} CSS Custom Properties: ${customPropValue.trim() === 'test-value' ? 'Working' : 'Fallback Required'}`);
...
console.log(`  🏃 Reduced Motion: ${prefersReducedMotion ? 'Enabled (Accessibility)' : 'Disabled'}`);
console.log(`  🎨 High Contrast: ${prefersHighContrast ? 'Enabled' : 'Disabled'}`);
...
if ('performance' in window && window.performance.timing) {
    ...
    const firstPaint = window.performance.getEntriesByType('paint')[0]?.startTime || 'N/A';
    
    console.log(`  ⏱️  Total Load Time: ${loadTime}ms`);
    console.log(`  🏗️  DOM Content Loaded: ${domContentLoaded}ms`);
    console.log(`  🎨 First Paint: ${typeof firstPaint === 'number' ? Math.round(firstPaint) + 'ms' : firstPaint}`);
} else {
...
setTimeout(() => {
    console.log(`  ${errorCount === 0 ? '✅' : '⚠️'} Console Errors: ${errorCount} detected`);
...
console.log(`  📊 System Health: ${workingSystems}/${totalSystems} critical systems operational`);
console.log(`  🎯 Browser Compatibility: ${Object.values(capabilities).filter(Boolean).length}/${Object.keys(capabilities).length} features supported`);
console.log(`  📱 Mobile Optimized: ${isMobile ? 'Yes' : 'Desktop Mode'}`);
console.log(`  ♿ Accessibility: ${prefersReducedMotion ? 'Motion-Reduced Mode' : 'Full Effects Mode'}`);
...
console.log('💡 Tip: Run this script on different pages and browsers for comprehensive testing.');
...
window.portfolioValidationResults = {
    ...
    browserSupport: `${Object.values(capabilities).filter(Boolean).length}/${Object.keys(capabilities).length}`
};
