...
(function() {
  ...
  const TIMING = {
    ...
    SYSTEM_COOLDOWNS: {
      /* System-specific cooldown periods (ms) */ TITLE: 1200,
      NAV: 900,
    ...
    STARTUP_DELAY: {
      /* Staggered startup delays */ TITLE: 800,
      NAV: 1500,
      EFFECTS: 2500,
      BACKGROUND: 300
    },
  ...
  const CRTDirector = {
    ...
    init() {
      ...
      console.log(
        '[Director] Initialized with performance level:',
    ...
    canRunEffect(effectType, intensity = 0.5) {
      ...
      const cooldownTime =
        TIMING.SYSTEM_COOLDOWNS[effectType] || TIMING.MINIMUM_COOLDOWN;
      ...
      if (intensity > 0.7) {
        const highIntensityLastRun =
          state.lastEffectTimes.get('HIGH_INTENSITY') || 0;
        if (
          Date.now() - highIntensityLastRun <
          RATE_LIMITS.HIGH_INTENSITY_COOLDOWN
        ) {
          return false;
        }
      } else if (intensity > 0.4) {
        const mediumIntensityLastRun =
          state.lastEffectTimes.get('MEDIUM_INTENSITY') || 0;
        if (
          Date.now() - mediumIntensityLastRun <
          RATE_LIMITS.MEDIUM_INTENSITY_COOLDOWN
        ) {
          return false;
        }
      } else {
        const lowIntensityLastRun =
          state.lastEffectTimes.get('LOW_INTENSITY') || 0;
        if (
          Date.now() - lowIntensityLastRun <
          RATE_LIMITS.LOW_INTENSITY_COOLDOWN
        ) {
          return false;
        }
      }
      ...
      if (state.activeEffects.size >= TIMING.MAXIMUM_SIMULTANEOUS) {
        return false;
      }
    ...
    handleEffectStart(event) {
      ...
      console.log(
        `[Director] Effect started: ${effectType} (${intensity.toFixed(2)}) from ${source}`
      );
