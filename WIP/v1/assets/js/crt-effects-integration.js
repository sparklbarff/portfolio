...
(function() {
  ...
  function init() {
    ...
    console.log("[Integration] Bridging legacy CRT effects to optimized system");
    
    // Connect to legacy system events
    if (legacySystemAvailable && typeof window.CRTSystem.addEventListener === 'function') {
      window.CRTSystem.addEventListener('effectsEnabled', handleLegacyEffectsEnabled);
      window.CRTSystem.addEventListener('effectsDisabled', handleLegacyEffectsDisabled);
      window.CRTSystem.addEventListener('effectTriggered', handleLegacyEffectTriggered);
    }
    ...
    if (performanceMonitorAvailable && typeof window.CRTSystem.getPerformanceLevel === 'function') {
  ...
  function handleLegacyEffectTriggered(event) {
    ...
    const mappedEffect = Object.entries(EFFECT_MAPPING).find(
      ([legacyClass, _]) => legacyClass.includes(effectName) || effectName.includes(legacyClass)
    );
