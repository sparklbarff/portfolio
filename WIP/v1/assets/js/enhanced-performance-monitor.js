...
(function() {
  ...
  function trackFrame(timestamp) {
    ...
    const avgFrameTime = metrics.frameTimes.reduce((sum, time) => sum + time, 0) / metrics.frameTimes.length;
  ...
  function updatePerformanceLevel() {
    ...
    if (newLevel !== metrics.performanceLevel) {
      ...
      if (CONFIG.debugMode) {
        console.log(`[Performance] Switching to ${newLevel} performance mode (${avgFps.toFixed(1)} FPS)`);
      }
    }
  }
  ...
  function configureEffects(effectSettings) {
    ...
    Object.entries(effectSettings).forEach(([effect, enabled]) => {
      const elements = document.querySelectorAll(`.${effect}, .crt-${effect}, .vhs-${effect}`);
      elements.forEach(el => {
  ...
  function init() {
    ...
    if (window.performance && window.performance.memory) {
      setInterval(() => {
        metrics.memoryUsage = window.performance.memory.usedJSHeapSize / 1048576; // Convert to MB
