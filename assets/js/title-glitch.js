...
(function() {
  ...
  const TitleGlitchFactory = {
    create(elementId) {
      ...
      function calculateMagneticDistortion(letter, globalIntensity) {
        ...
        const magneticStrength = CRTConfig.getPhysicsParameter('TITLE', 'MAGNETIC_FIELD_STRENGTH') || 0.25;
        const perspectiveMax = CRTConfig.getPhysicsParameter('TITLE', 'PERSPECTIVE_MAX') || 25;
        ...
        const localPhase = localState.magneticPhase + (letter.normalizedX * Math.PI);
        const wearMultiplier = 1 + magneticWear * 100;
        const fieldIntensity = globalIntensity * magneticStrength * wearMultiplier;
        
        const primaryField = Math.sin(localPhase) * fieldIntensity;
        const secondaryField = Math.cos(localPhase * 1.3 + thermalStress * 10) * fieldIntensity * 0.7;
        const thermalDrift = Math.sin(localPhase * 0.3) * fieldIntensity * Math.abs(thermalStress) * 6;
        ...
        const zDisplace = localState.dimensionalDepth * 
      ...
      function createPhosphorTrail(letter, intensity) {
        ...
        if (sysState.isLowEndDevice || sysState.performanceLevel === 'low') return;
        ...
        }, 50, { purpose: 'phosphor-trail-decay' });
      }
      ...
      function createCharacterFragments(letter, intensity) {
        ...
        }, fragmentDuration, { purpose: 'character-fragment' });
      }
      ...
      function applyOptimizedGlitch(letter, baseIntensity) {
        ...
        const thresholds = {
          RGB_SEPARATION: CRTConfig.getThreshold('TITLE', 'RGB_SEPARATION') || 0.4,
          CHARACTER_REPLACE: CRTConfig.getThreshold('TITLE', 'CHARACTER_REPLACE') || 0.5,
          BRIGHTNESS_MOD: CRTConfig.getThreshold('TITLE', 'BRIGHTNESS_MOD') || 0.3,
          PHOSPHOR_TRAIL: CRTConfig.getThreshold('TITLE', 'PHOSPHOR_TRAIL') || 0.6,
          CASCADE_TRIGGER: CRTConfig.getThreshold('TITLE', 'CASCADE_TRIGGER') || 0.7,
          Z_MOVEMENT: CRTConfig.getThreshold('TITLE', 'Z_MOVEMENT') || 0.6,
          DECOMPOSITION: CRTConfig.getThreshold('TITLE', 'DECOMPOSITION') || 0.8
        };
        ...
        if (effectiveIntensity > thresholds.RGB_SEPARATION) {
          ...
          const convergenceBase = CRTConfig.getPhysicsParameter('TITLE', 'CONVERGENCE_ERROR_BASE') || 0.9;
          const wearPatterns = window.CRTSystem.getWearPatterns();
          const convergenceWear = wearPatterns.convergenceWear || 0;
          
          const convergenceError = convergenceBase * (1.5 + letter.normalizedX * 0.8);
          const redOffset = convergenceError * (1 + convergenceWear);
          const blueOffset = convergenceError * (1 + convergenceWear * 0.8);
          
          // Get color temperature from system
          const colorTempMin = CRTConfig.getPhysicsParameter('TITLE', 'COLOR_TEMP_MIN') || 6500;
          const colorTempMax = CRTConfig.getPhysicsParameter('TITLE', 'COLOR_TEMP_MAX') || 9300;
          const colorTemperature = colorTempMin + 
            (colorTempMax - colorTempMin) * sysState.thermalLevel;
          
          // Convert color temperature to RGB values
          const tempRGB = colorTemperatureToRGB(colorTemperature);
          const redIntensity = tempRGB.r / 255;
          const greenIntensity = tempRGB.g / 255;
          const blueIntensity = tempRGB.b / 255;
          
          // Reduce effect intensity by 20%
          const shadowLayers = [
            `${redOffset * 0.8}px 0 rgba(255, ${64 * redIntensity}, ${32 * redIntensity}, ${effectiveIntensity * 0.7})`,
            `${-blueOffset * 0.8}px 0 rgba(${64 * blueIntensity}, ${128 * blueIntensity}, 255, ${effectiveIntensity * 0.7})`,
            `0 0 ${effectiveIntensity * 12}px rgba(${tempRGB.r}, ${tempRGB.g}, ${tempRGB.b}, ${effectiveIntensity * 0.5})`,
            `0 0 ${effectiveIntensity * 24}px rgba(0, 255, 200, ${effectiveIntensity * 0.25})`
          ];
        ...
        if (effectiveIntensity > thresholds.CHARACTER_REPLACE) {
          ...
          if (Math.random() < replaceChance) {
            ...
            const restoreDelay = CRTConfig.getEffectDuration('CHARACTER_RESTORE') || 120;
            ...
            }, restoreDelay + Math.random() * 40, { purpose: 'character-restore' });
          }
        }
        ...
        if (effectiveIntensity > thresholds.PHOSPHOR_TRAIL) {
          ...
          }, 200 + Math.random() * 150, { purpose: 'phosphor-glow' });
          
          // Track phosphor wear in unified system
          // TODO: Implement per-letter wear tracking in unified system
        }
        ...
        if (effectiveIntensity > thresholds.DECOMPOSITION && 
        ...
        if (effectiveIntensity > thresholds.CASCADE_TRIGGER && Math.random() < 0.5) {
      ...
      function runOptimizedGlitchSystem(currentTime) {
        ...
        // Get glitch cooldown from config
        const glitchCooldown = CRTConfig.getSystemCooldown('TITLE_GLITCH') || 800;
        ...
        if (localState.activeGlitchCount > 0 || !window.CRTSystem.requestEffect('TITLE', 0.5, SYSTEM_ID)) {
          localState.animationFrameId = requestAnimationFrame(runOptimizedGlitchSystem);
          CRTResource.registerAnimation(localState.animationFrameId);
          return;
        }
        ...
        const glitchProbability = CRTConfig.EFFECT_PROBABILITIES.TITLE.BASE_GLITCH_CHANCE || 0.1;
        
        // Only apply glitches with reduced probability
        if (Math.random() < glitchProbability) {
          ...
          letters.forEach(letter => {
            ...
            const minGlitchInterval = 400;
            
            // Reduced glitch chance
            const organicGlitchChance = 0.1 + 
            ...
            if (shouldGlitch) {
              const localIntensity = baseIntensity * (0.6 + Math.random() * 0.5);
      ...
      function start() {
        ...
        const cascadeHandler = (event) => {
          ...
          if (intensity > 0.6) {
            ...
            const dimensionShiftDuration = CRTConfig.getEffectDuration('DIMENSION_SHIFT') || 200;
            
            CRTResource.setTimeout(() => {
              localState.dimensionalDepth *= 0.5;
            }, dimensionShiftDuration, { purpose: 'dimension-shift' });
          }
        };
  ...
  function initializeTitleGlitch() {
    ...
    if (!window.CRTConfig) {
      console.warn('[Title] CRTConfig not available, system may not function properly');
    }
    
    if (!window.CRTResource) {
      console.warn('[Title] CRTResource not available, system may not clean up properly');
    }
