...
(function () {
  ...
  const NavGlitchFactory = {
    create(elementId) {
      ...
      function applyTrackingError() {
        ...
        if (localState.trackingErrors.length >= 2 || sysState.isLowEndDevice)
          return;

        // Get tracking error amplitude from config
        const trackingErrorAmplitude =
          CRTConfig.getPhysicsParameter("NAV", "TRACKING_ERROR_AMPLITUDE") || 4;
        ...
        const trackingErrorDuration =
          CRTConfig.getEffectDuration("TRACKING_ERROR") || 500;
      ...
      function applyNavColorEffects(linkState, effectIntensity) {
        ...
        const rgbSeparationThreshold =
          CRTConfig.getThreshold("NAV", "RGB_SEPARATION") || 0.3;
      ...
      function applyNavDropoutEffect(linkState, intensity) {
        ...
        const dropoutThreshold =
          CRTConfig.getThreshold("NAV", "DROPOUT_MIN") || 0.7;

        const sysState = window.CRTSystem.getState();

        if (
          linkState.vhsState.dropoutActive ||
      ...
      function applyNavColorBleeding(linkState, effectIntensity) {
        ...
        const colorBleedRange =
          CRTConfig.getPhysicsParameter("NAV", "COLOR_BLEED_RANGE") || 2;
      ...
      function applyNavInterlacing(linkState, effectIntensity) {
        ...
        const interlaceFlickerChance =
          CRTConfig.getPhysicsParameter("NAV", "INTERLACE_FLICKER_CHANCE") ||
          0.4;
          ...
          const flickerIntervalId = CRTResource.setInterval(
            () => {
              linkState.element.style.opacity =
                linkState.element.style.opacity === "1" ? "0.9" : "1";
              flickers++;
      ...
      function calculateMagneticDistortion(linkState, globalIntensity) {
        ...
        const magneticFieldStrength =
          CRTConfig.getPhysicsParameter("NAV", "MAGNETIC_FIELD_STRENGTH") ||
          0.4;
        ...
        const localPhase =
          localState.globalMagneticPhase + linkState.normalizedX * Math.PI;
        const wearMultiplier = 1 + magneticWear * 120;
        const fieldIntensity =
          globalIntensity * magneticFieldStrength * wearMultiplier;
        ...
        const secondaryField =
          Math.cos(localPhase * 1.3 + thermalStress * 12) *
          fieldIntensity *
          0.7;
        const thermalDrift =
          Math.sin(localPhase * 0.3) * fieldIntensity * thermalStress * 8;
        const chaosField =
          Math.sin(localPhase * 2.7 + Math.random() * Math.PI) *
          fieldIntensity *
          0.5;

        // Implementation: Enhanced displacement calculations
        const xDisplace =
          (primaryField + secondaryField + thermalDrift + chaosField) * 40;
        const yDisplace =
          (secondaryField * 0.8 + thermalDrift + chaosField * 0.6) * 24;
        const rotation =
          (primaryField * 0.3 + thermalDrift + chaosField * 0.4) * 18;
      ...
      function calculateCursorInfluence(linkState) {
        ...
        if (localState.cursorPosition.x < 0 || sysState.isLowEndDevice)
          return { x: 0, y: 0 };

        // Get cursor influence parameters from config
        const cursorInfluenceRadius =
          CRTConfig.getPhysicsParameter("NAV", "CURSOR_INFLUENCE_RADIUS") ||
          150;
        const cursorStrength =
          CRTConfig.getPhysicsParameter("NAV", "CURSOR_STRENGTH") || 0.25;
      ...
      function calculateAudioReactivity(linkState) {
        ...
        if (localState.audioAmplitude < 0.05 || sysState.isLowEndDevice)
          return 0;

        // Get audio reactivity parameter from config
        const audioReactivity =
          CRTConfig.getPhysicsParameter("NAV", "AUDIO_REACTIVITY") || 0.04;
        ...
        const waveEffect = Math.sin(
          localState.globalMagneticPhase * 3 + phaseOffset
        );

        // Calculate reactivity intensity
        const reactivity =
          localState.audioAmplitude *
          audioReactivity *
          (0.5 + Math.abs(waveEffect) * 0.5);
        ...
        if (reactivity > linkState.audioReactiveState.peakIntensity) {
          ...
          const timeSincePeak =
            performance.now() - linkState.audioReactiveState.lastPeak;
      ...
      function updateElectronBeamFocus(linkState, isFocused) {
        ...
        const electronFocusSpeed =
          CRTConfig.getPhysicsParameter("NAV", "ELECTRON_FOCUS_SPEED") || 0.15;
        ...
        const newIntensity =
          currentIntensity +
          (targetIntensity - currentIntensity) * electronFocusSpeed;
        ...
        if (newIntensity > 0.01) {
          ...
          linkState.element.style.textShadow = `
            0 0 ${4 * glowIntensity}px rgba(232, 227, 216, ${
            0.7 * glowIntensity
          }),
          ...
          const focusTransitionTime =
            CRTConfig.TIMING.DURATIONS.ELECTRON_FOCUS || 300;
          ...
          linkState.element.style.filter = `brightness(${
            1 + 0.3 * glowIntensity
          })`;
      ...
      function applyOptimizedDistortion(linkState, baseIntensity) {
        ...
        const thresholds = {
          RGB_SEPARATION:
            CRTConfig.getThreshold("NAV", "RGB_SEPARATION") || 0.3,
          CHARACTER_REPLACE:
            CRTConfig.getThreshold("NAV", "CHARACTER_REPLACE") || 0.4,
          BRIGHTNESS_MOD:
            CRTConfig.getThreshold("NAV", "BRIGHTNESS_MOD") || 0.2,
          CASCADE_TRIGGER:
            CRTConfig.getThreshold("NAV", "CASCADE_TRIGGER") || 0.6,
          MAGNETIC_ATTRACTION:
            CRTConfig.getThreshold("NAV", "MAGNETIC_ATTRACTION") || 0.1,
          TRACKING_ERROR:
            CRTConfig.getThreshold("NAV", "TRACKING_ERROR") || 0.4,
          DROPOUT_MIN: CRTConfig.getThreshold("NAV", "DROPOUT_MIN") || 0.7,
        };

        let effectiveIntensity = baseIntensity;

        /* Apply physics engine thermal effects */
        if (window.CRTPhysics) {
          const thermalState = window.CRTPhysics.getThermalState();
          effectiveIntensity *= 1 + thermalState.warmupProgress * 0.2;

          // Apply physics-based thermal styling
          window.CRTPhysics.applyThermalEffects(linkState.element);
        }
        ...
        /* Apply intensity factor from system with fallback */
        const intensityFactor = window.CRTSystem
          ? window.CRTSystem.getIntensityFactor()
          : 1.0;
        effectiveIntensity *= intensityFactor;
        ...
        const totalX =
          ...
          cursorInfluence.x * 40 +
          audioReactivity * 15 * (Math.random() - 0.5);
        const totalY =
          distortion.yDisplace +
          cursorInfluence.y * 40 +
          audioReactivity * 10 * (Math.random() - 0.5);
        const totalRotation =
          distortion.rotation + audioReactivity * 20 * (Math.random() - 0.5);

        linkState.element.style.transform = `translate(${totalX}px, ${totalY}px) rotate(${totalRotation}deg)`;
        ...
        // Enhanced RGB separation effects with physics-based convergence (Task 1.4)
        if (
          effectiveIntensity > thresholds.RGB_SEPARATION ||
          audioReactivity > 0.1
        ) {
          let convergenceError = { red: {}, green: {}, blue: {} };

          // Use enhanced physics engine convergence with thermal effects
          if (window.CRTPhysics) {
            convergenceError = window.CRTPhysics.getConvergenceError();
            const thermalState = window.CRTPhysics.getThermalState();
            const scanPosition = window.CRTPhysics.getScanPosition();

            // Apply enhanced thermal and magnetic effects for navigation
            const cursorDistance = Math.sqrt(
              Math.pow(
                localState.cursorPosition.x - linkState.element.offsetLeft,
                2
              ) +
                Math.pow(
                  localState.cursorPosition.y - linkState.element.offsetTop,
                  2
                )
            );
            const magneticInfluence = Math.max(0, 1 - cursorDistance / 200); // Magnetic field within 200px

            try {
              const thermalMultiplier =
                1 + (thermalState.temperature - 22) * 0.015; // Stronger thermal effect for nav
              const magneticMultiplier = 1 + magneticInfluence * 0.4; // Magnetic distortion
              const positionMultiplier = 1 + linkState.normalizedX * 0.3; // Position-based error

              let redShadow =
                convergenceError.red?.textShadow ||
                "0.5px 0 rgba(255, 64, 32, 0.7)";
              let blueShadow =
                convergenceError.blue?.textShadow ||
                "-0.5px 0 rgba(64, 128, 255, 0.7)";

              // Enhanced convergence calculation with multiple physics factors
              if (redShadow) {
                const match = redShadow.match(/([-\d.]+)px/);
                ...
                const enhancedOffset =
                  ...
                  magneticMultiplier *
                  positionMultiplier;
                redShadow = `${enhancedOffset}px ${
                  magneticInfluence * 0.5
                }px rgba(255, 64, 32, ${effectiveIntensity * 0.8})`;
              }

              if (blueShadow) {
                const match = blueShadow.match(/([-\d.]+)px/);
                const baseOffset = match ? parseFloat(match[1]) : -0.5;
                const enhancedOffset =
                  baseOffset *
                  thermalMultiplier *
                  magneticMultiplier *
                  positionMultiplier;
                blueShadow = `${enhancedOffset}px ${
                  -magneticInfluence * 0.3
                }px rgba(64, 128, 255, ${effectiveIntensity * 0.8})`;
              }

              const shadowLayers = [
                redShadow,
                blueShadow,
                `0 0 ${effectiveIntensity * 18}px rgba(232, 227, 216, ${
                  effectiveIntensity * 0.6
                })`,
                `0 0 ${effectiveIntensity * 35}px rgba(0, 255, 200, ${
              ...
              console.warn("[Nav] Physics convergence error:", error);
              // Fall back to basic convergence
              const shadowLayers = [
                `0.8px 0 rgba(255, 64, 32, ${effectiveIntensity * 0.7})`,
                `-0.8px 0 rgba(64, 128, 255, ${effectiveIntensity * 0.7})`,
                `0 0 ${effectiveIntensity * 18}px rgba(232, 227, 216, ${
                  effectiveIntensity * 0.6
                })`,
                `0 0 ${effectiveIntensity * 35}px rgba(0, 255, 200, ${
            ...
            const convergenceBase =
              CRTConfig.getPhysicsParameter("NAV", "CONVERGENCE_ERROR_BASE") ||
              1.5;

            const redOffset =
              convergenceBase * (1.5 + linkState.normalizedX * 0.6);
            const blueOffset =
              convergenceBase * (1.5 + (1 - linkState.normalizedX) * 0.6);
        ...
        if (effectiveIntensity > thresholds.CHARACTER_REPLACE) {
          ...
          if (Math.random() < replaceChance) {
            ...
            const restoreTimer = CRTResource.setTimeout(
              ...
              CRTConfig.getEffectDuration("CHARACTER_RESTORE") ||
                120 + Math.random() * 60,
        ...
        if (
          effectiveIntensity > thresholds.DROPOUT_MIN &&
          Math.random() < 0.2
        ) {
      ...
      function runOptimizedGlitchSystem(currentTime) {
        ...
        if (sysState.performanceLevel !== "low") {
          ...
          localState.globalMagneticPhase += sysState.isLowEndDevice
            ? 0.03
            : 0.055;
        }
        ...
        const trackingErrorProbability =
          CRTConfig.EFFECT_PROBABILITIES.NAV?.TRACKING_ERROR_CHANCE || 0.0008;
        ...
        const maxGlitches = sysState.isLowEndDevice
          ? 2
          : CRTConfig.PERFORMANCE_LEVELS[
              sysState.performanceLevel.toUpperCase()
        ...
        const glitchProbability =
          CRTConfig.EFFECT_PROBABILITIES.NAV?.BASE_GLITCH_CHANCE || 0.18;

        linkStates.forEach((linkState) => {
          const timeSinceLastGlitch =
            currentTime - linkState.glitchState.lastGlitch;
      ...
      function start() {
        ...
        if (!window.CRTSystem || !window.CRTSystem.isInitialized()) {
          console.warn(
            "[Nav] CRTSystem not ready, attempting fallback initialization"
          );
        ...
        try {
          ...
          console.error("[Nav] Failed to register with CRTSystem:", error);
          // Emergency fallback
          localState.animationFrameId = requestAnimationFrame(
            runOptimizedGlitchSystem
          );
        ...
        if (window.CRTPhysics) {
          const physicsHandler = (event) => {
            const { thermal, convergence } = event.detail;

            // Update local state based on physics
            if (thermal && thermal.warmupProgress > 0.5) {
              // Increase magnetic attraction during thermal stress
              linkStates.forEach((linkState) => {
                linkState.magneticState.attraction *=
                  1 + thermal.warmupProgress * 0.3;
              });
            }
          };

          window.addEventListener("crtPhysicsUpdate", physicsHandler);
          CRTResource.registerListener(
            window,
            "crtPhysicsUpdate",
            physicsHandler
          );
        ...
        const audioHandler = (event) => {
          if (event.detail && typeof event.detail.amplitude === "number") {
            localState.audioAmplitude =
              localState.audioAmplitude * 0.9 + event.detail.amplitude * 0.1;
          }
        };
  ...
  function initializeNavGlitch() {
    ...
    if (!window.CRTConfig) {
      console.warn(
        "[Nav] CRTConfig not available, system may not function properly"
      );
    }

    if (!window.CRTResource) {
      console.warn(
        "[Nav] CRTResource not available, system may not clean up properly"
      );
    }
