...
(function() {
  ...
  const BG_CONSTANTS = {
    ...
    PRELOAD_COUNTS: { LOW: 1, MEDIUM: 2, HIGH: 3 },  /* Images to preload by performance */
  ...
  function analyzeBackgroundIntensity(imageSrc) {
    ...
    img.onload = function() {
      ...
      ctx.drawImage(img, 0, 0, BG_CONSTANTS.SAMPLE_SIZE, BG_CONSTANTS.SAMPLE_SIZE);
      
      try {
        const imageData = ctx.getImageData(0, 0, BG_CONSTANTS.SAMPLE_SIZE, BG_CONSTANTS.SAMPLE_SIZE);
  ...
  function fetchManifest() {
    console.log('[Background] Fetching manifest from:', window.location.origin + '/manifest.json');
  ...
  function generateImagePaths() {
    ...
    console.log('[Background] Generated image paths:', results.slice(0, 3), '... (' + results.length + ' total)');
    return results;
  }
  ...
  function preloadImage(src) {
    ...
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        console.log(`[Background] Preloaded: ${src} (${img.naturalWidth}x${img.naturalHeight})`);
        resolve(img);
      };
  ...
  function preloadNext(count) {
    ...
    const isSlowConnection = connection && (connection.saveData || 
      connection.effectiveType === 'slow-2g' || connection.effectiveType === '2g');
    
    let adjustedCount = BG_CONSTANTS.PRELOAD_COUNTS[performanceLevel.toUpperCase()] || count;
    
    // Further reduce preload on slow connections
    if (isSlowConnection) {
      adjustedCount = Math.max(1, Math.floor(adjustedCount * 0.5));
      console.log(`[Background] Reduced preload for slow connection: ${adjustedCount}`);
    }
  ...
  function setupCycle() {
    ...
    if (document.documentElement.dataset.motion !== "paused") {
      const period = parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--period')) * 1000 || BG_CONSTANTS.DEFAULT_PERIOD;
  ...
  function setupFirstBg() {
    ...
    return preloadImage(firstSrc)
      ...
      .catch(error => {
        console.error('[Background] Error setting up initial background:', error);
        ...
        fallback.style.filter = 'none';
        bgContainer.appendChild(fallback);
        console.log('[Background] Created lighter emergency fallback background');
  ...
  const cascadeHandler = (event) => {
    try {
      ...
      if (intensity > 0.5) {
        const sweepTimer = setTimeout(() => triggerScanlineSweep(), Math.random() * 500);
