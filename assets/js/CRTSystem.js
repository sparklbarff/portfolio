...
(function() {
  ...
  const CRTSystem = {
     ...
     */
    init() {
      if (globalState.systemInitialized) return this;
      ...
      console.log('[CRTSystem] Initialized with performance level:', globalState.performanceLevel);
      return this;
    },
    ...
    loadDefaultConfig() {
      ...
      return {
        ...
        PERFORMANCE_LEVELS: {
          LOW: { effectProbability: 0.4, cooldownMultiplier: 1.6, intensityMultiplier: 0.5 },
          MEDIUM: { effectProbability: 0.7, cooldownMultiplier: 1.2, intensityMultiplier: 0.8 },
          HIGH: { effectProbability: 1.0, cooldownMultiplier: 1.0, intensityMultiplier: 1.0 }
        },
     ...
     */
    loadDefaultResource() {
      console.warn('[CRTSystem] CRTResource not found, using minimal implementation');
      return {
        registerTimer: () => {},
        registerElement: () => {},
        registerListener: () => {},
    ...
    updatePerformanceSettings() {
      ...
      const settings = CRTConfig.PERFORMANCE_LEVELS[level.toUpperCase()] || 
                      CRTConfig.PERFORMANCE_LEVELS.HIGH;
      
      globalState.performanceSettings = settings;
      
      // Set target FPS based on performance level
      globalState.targetFPS = CRTConfig.FRAME_RATES[level.toUpperCase()] || 
                             CRTConfig.FRAME_RATES.HIGH;
      
      // Broadcast frame rate change
      window.dispatchEvent(new CustomEvent('frameRateChange', {
        detail: { 
    ...
    setupEventListeners() {
      ...
      const observer = new MutationObserver(mutations => {
        mutations.forEach(mutation => {
          if (mutation.attributeName === 'data-motion') {
            ...
            window.dispatchEvent(new CustomEvent('motionStateChange', {
      ...
      window.addEventListener('audioData', event => {
        if (event.detail && typeof event.detail.amplitude === 'number') {
          ...
          globalState.audioAmplitude = globalState.audioAmplitude * 0.8 + 
                                      event.detail.amplitude * 0.2;
          ...
          if (!this.audioThrottled && globalState.audioAmplitude > 0.05) {
            ...
            setTimeout(() => {
              window.dispatchEvent(new CustomEvent('crtAudioUpdate', {
      ...
      window.addEventListener('effectRequest', this.handleEffectRequest.bind(this));
      window.addEventListener('effectComplete', this.handleEffectComplete.bind(this));
      window.addEventListener('cascadeEvent', this.handleCascadeEvent.bind(this));
    ...
    initializeWearPatterns() {
      ...
      const wearInterval = setInterval(() => {
        ...
        globalState.wearPatterns.magneticWear = Math.min(globalState.wearPatterns.magneticWear, 0.05);
        globalState.wearPatterns.thermalStress = Math.max(-0.08, Math.min(globalState.wearPatterns.thermalStress, 0.08));
        globalState.wearPatterns.powerSupplyWear = Math.min(globalState.wearPatterns.powerSupplyWear, 0.04);
        globalState.wearPatterns.convergenceWear = Math.min(globalState.wearPatterns.convergenceWear, 0.06);
        globalState.wearPatterns.trackingWear = Math.min(globalState.wearPatterns.trackingWear, 0.07);
        
        // Update global wear average
        const wearValues = Object.values(globalState.wearPatterns);
        globalState.globalWear = wearValues.reduce((sum, val) => sum + Math.abs(val), 0) / wearValues.length;
        
        // Update thermal level based on thermal stress
        this.updateThermalLevel(Math.abs(globalState.wearPatterns.thermalStress) / 0.08);
        
        // Broadcast wear update at a slow rate
        if (Math.random() < 0.1) {
          window.dispatchEvent(new CustomEvent('wearUpdate', {
            detail: { 
    ...
    handleEffectRequest(event) {
      ...
      if (!this.canRunEffect(effectType, intensity)) {
        ...
        window.dispatchEvent(new CustomEvent('effectRejected', {
      ...
      // Record effect start
      pendingEffects.set(effectType, { intensity, source, systemId, time: Date.now() });
      ...
      globalState.activeIntensity = Math.max(globalState.activeIntensity, intensity);
      
      console.log(`[CRTSystem] Effect approved: ${effectType} (${intensity.toFixed(2)}) from ${source}`);
      
      // Approve effect request
      window.dispatchEvent(new CustomEvent('effectApproved', {
        detail: { 
          effectType, 
    ...
    canRunEffect(effectType, intensity = 0.5) {
      ...
      const settings = globalState.performanceSettings || 
                      CRTConfig.PERFORMANCE_LEVELS.MEDIUM;
      ...
      const cooldownTime = CRTConfig.TIMING.SYSTEM_COOLDOWNS[effectType] || 
      ...
      if (intensity > 0.7) {
        ...
        if (Date.now() - highIntensityLastRun < CRTConfig.RATE_LIMITS.HIGH_INTENSITY_COOLDOWN) {
          return false;
        }
      } else if (intensity > 0.4) {
        const mediumIntensityLastRun = lastEffectTimes.get('MEDIUM_INTENSITY') || 0;
        if (Date.now() - mediumIntensityLastRun < CRTConfig.RATE_LIMITS.MEDIUM_INTENSITY_COOLDOWN) {
          return false;
        }
      } else {
        const lowIntensityLastRun = lastEffectTimes.get('LOW_INTENSITY') || 0;
        if (Date.now() - lowIntensityLastRun < CRTConfig.RATE_LIMITS.LOW_INTENSITY_COOLDOWN) {
          return false;
        }
      }
      ...
      if (activeEffects.size >= MAXIMUM_SIMULTANEOUS) return false;
    ...
    adaptEffectIntensity(performanceRatio) {
      ...
      document.documentElement.style.setProperty('--adaptive-intensity', intensityScale);
      document.documentElement.style.setProperty('--adaptive-opacity', intensityScale * 0.8);
      document.documentElement.style.setProperty('--adaptive-animation-speed', Math.max(0.5, performanceRatio));
      
      console.log(`[CRTSystem] Adapted effect intensity: ${intensityScale.toFixed(2)}`);
    },
    
    /*
     * Request an effect through the unified system
     */
    requestEffect(effectType, intensity = 0.5, source = 'unknown', systemId = null) {
      if (!this.canRunEffect(effectType, intensity)) {
        return false;
      }
      
      // Broadcast effect request
      window.dispatchEvent(new CustomEvent('effectRequest', {
        detail: { 
          effectType, 
          intensity, 
          source,
    ...
    completeEffect(effectType, source = 'unknown', systemId = null) {
      window.dispatchEvent(new CustomEvent('effectComplete', {
        detail: { effectType, source, systemId }
      }));
    },
    ...
    triggerCascade(intensity, origin = 'unknown') {
      ...
      console.log(`[CRTSystem] Cascade triggered: ${intensity.toFixed(2)} from ${origin}`);
      ...
      const recoveryTimer = setTimeout(() => {
        ...
        if (globalState.cascadeLevel < 0.1) {
          ...
          // Broadcast recovery
          window.dispatchEvent(new CustomEvent('crtRecovery', {
     ...
     */
    isInGlobalCooldown() {
      if (!this._globalCooldownEnd) return false;
